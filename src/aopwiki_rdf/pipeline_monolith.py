...
Inlined monolith body with dynamic code execution eliminated. All
configuration is read from PipelineConfig fields directly. The original
monolith script (AOP-Wiki_XML_to_RDF_conversion.py) is preserved as backup.
"""
...
def main(config: PipelineConfig | None = None) -> None:
    ...
    def download_with_retry(url, filename, max_retries=None):
    ...
    try:
        download_with_retry(config.aopwiki_xml_url, aopwikixmlfilename)
    except requests.RequestException as e:
    ...
    def map_chemicals_batch(cas_numbers, batch_size=100, bridgedb_url=None, timeout=None):
        """
    ...
    try:
        ...
        urllib.request.urlretrieve(config.promapping_url, filepath + pro)
    ...
    # --- HGNC Download (wired from hgnc.download module) ---
    from aopwiki_rdf.hgnc import download_hgnc_data as _download_hgnc
    hgnc_cache_path = config.data_dir / 'HGNCgenes.txt'
    _hgnc_content = _download_hgnc(
    ...
    logger.info("HGNC data downloaded/cached successfully")
    ...
    def map_genes_batch_bridgedb(gene_list, chunk_size=100):
        ...
        for chunk_idx in range(0, len(gene_list), chunk_size):
            ...
            try:
                ...
                response = requests.post(batch_url, data=batch_data, headers=headers, timeout=config.request_timeout)
                response.raise_for_status()
                ...
                for gene in chunk:
                    try:
                        response = requests.get(bridgedb + 'xrefs/H/' + gene[5:], timeout=config.request_timeout)
                        response.raise_for_status()
    ...
    try:
        response = requests.get(bridgedb + 'properties', timeout=config.request_timeout)
        response.raise_for_status()
